...
class Local extends Base {
	...
	public function install_paths() {
		...
		$dst_folder = \trailingslashit( EWWW_IMAGE_OPTIMIZER_TOOL_PATH );
		if ( PHP_OS === 'WINNT' ) {
		...
		$this->debug_message( "generated paths:\n$jpegtran_src\n$optipng_src\n$gifsicle_src\n$pngquant_src\n$webp_src\n$jpegtran_dst\n$optipng_dst\n$gifsicle_dst\n$pngquant_dst\n$webp_dst" );
	...
	public function install_tools() {
		$this->debug_message( '<b>' . __METHOD__ . '()</b>' );
		$this->debug_message( 'Checking/Installing tools in ' . $this->tool_dir );
		$this->skip_tools();
		$toolfail = false;
		if ( ! \is_dir( $this->tool_dir ) && \is_writable( \dirname( $this->tool_dir ) ) ) {
			$this->debug_message( 'folder does not exist, creating...' );
			if ( ! \ewwwio_mkdir( $this->tool_dir ) ) {
				$this->debug_message( 'could not create folder' );
				return;
			}
		} elseif ( \is_dir( $this->tool_dir ) ) {
			if ( ! \is_writable( $this->tool_dir ) ) {
				$this->debug_message( $this->tool_dir . ' is not writable, not installing anything' );
				return;
			} elseif ( ! \is_executable( $this->tool_dir ) && PHP_OS !== 'WINNT' ) {
				$this->debug_message( $this->tool_dir . ' is not executable (non-Windows), not installing anything' );
				return;
			} elseif ( ! \is_readable( $this->tool_dir ) ) {
				$this->debug_message( $this->tool_dir . ' is not readable, not installing anything' );
				return;
			}
			$ewww_perms = \substr( \sprintf( '%o', \fileperms( $this->tool_dir ) ), -4 );
			$this->debug_message( $this->tool_dir . " permissions: $ewww_perms" );
		} else {
			$this->debug_message( \dirname( $this->tool_dir ) . ' is not writable, and the tools/ folder does not exist' );
			return;
		}
		list(
			$jpegtran_src,
		...
		if ( $toolfail ) {
			$this->tool_installation_failed_notice();
		}
	}
	...
	public function find_win_binary( $binary ) {
		...
		$tool_path = \trailingslashit( $this->tool_dir );
		...
		switch ( $tool ) {
			...
			case 'jpegtran-blind':
				...
				if ( 0 < $testjpgsize && $testjpgsize < 5700 ) {
					$this->debug_message( 'optimizer found' );
					return \__( 'unknown', 'ewww-image-optimizer' );
				}
				break;
			case 'optipng':
