...
//	echo ewww_image_optimizer_get_option( 'ewww_image_optimizer_skip_bundle' ) . "bundle skipper\n";
	if ( EWWW_IMAGE_OPTIMIZER_CLOUD ) {
		ewwwio_debug_message( 'cloud options enabled, shutting off binaries' );
		ewww_image_optimizer_disable_tools();
	}
...
function ewww_image_optimizer_install_tools () {
	...
	if ( ! is_dir( EWWW_IMAGE_OPTIMIZER_TOOL_PATH ) && is_writable( dirname( EWWW_IMAGE_OPTIMIZER_TOOL_PATH ) ) ) {
		...
		if ( ! is_writable( EWWW_IMAGE_OPTIMIZER_TOOL_PATH ) ) {
			ewwwio_debug_message( 'tool folder is not writable, not installing anything' );
			return;
		}
		$ewww_perms = substr( sprintf( '%o', fileperms( EWWW_IMAGE_OPTIMIZER_TOOL_PATH ) ), -4 );
		ewwwio_debug_message( "tool folder permissions: $ewww_perms" );
	} else {
		ewwwio_debug_message( 'parent folder is not writable, and the tools/ folder does not exist' );
		return;
	}
...
function ewww_image_optimizer_old_mimetype( $path, $case ) {
	ewwwio_debug_message( '<b>' . __FUNCTION__ . '()</b>' );
...
function ewww_image_optimizer_mimetype( $path, $category ) {
	...
	if ( 'i' === $category && strpos( $path, 's3' ) === 0 ) {
